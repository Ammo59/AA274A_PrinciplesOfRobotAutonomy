...
class NavigationNode(BaseNavigator):
    ...
    def compute_trajectory_plan(self, state: TurtleBotState, goal: TurtleBotState,
        ...
        astar = AStar((-horizon, -horizon), (horizon,horizon), initial_state, goal_state, occupancy, resolution=resolution)
        ...
        for i in range(len(path) - 1):
            delta_t[i] = np.linalg.norm(path[i+1] - path[i], axis=-1) / self.V_desired # axis = -1 needed to segregate tuples x and y coordinates norm calc 
